...
async function updatePrOnServer(updatedPr: Pr): Promise<boolean> {
...
export const PrEditor = ({ pr }: { pr: Pr }) => {
  ...
  function handleSave(e: MouseEvent<HTMLButtonElement, MouseEvent>) {
    ...
    updatePrOnServer(updatedPr as Pr).then((ok) => setHasSaved(ok));
  }
  ...
  return (
    <div>
      <Flex py={4}>
        <Button colorScheme={'twin'} onClick={handleSave}>
          Save
        </Button>
      </Flex>
      <Editor
